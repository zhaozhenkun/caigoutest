  ...
  _createClass(Toolbar, [{
    ...
    value: function _updateUIState() {
      ...
      if (this.hasPageLabels) {
        ...
        // alert(pageNumber)
        // alert(pageNumber)
        var startPage = window.PDFViewerApplication.baseUrl.split('&')[1]
        if (startPage) {
          startPage = startPage.split('=')[1]
        }
        var totalPage = window.PDFViewerApplication.baseUrl.split('&')[2]
        if (totalPage) {
          totalPage = totalPage.split('=')[1]
        }
        if (startPage && totalPage ) {
          document.getElementById('toolbarPercentage').innerText = parseInt(((parseInt(startPage) + parseInt(pageNumber) - 1) / totalPage) * 100)+'%'
        } else {
          document.getElementById('toolbarPercentage').innerText ='0%'
        }
